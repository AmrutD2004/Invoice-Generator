...
const Allinvoices = () => {
  ...
  const handleDelete = async (invIndex) => {
    if (window.confirm('Are you really want to delete invoice?')) {
      try {
        const response = await fetch(`https://invoicgenerator.pythonanywhere.com/api/delete-invoice/${invIndex}`, {
          method: "DELETE",
        })
        if (response.ok) {
  ...
  const handleStatusChange = async (invIndex) => {
    ...
    await axios.put(`https://invoicgenerator.pythonanywhere.com/api/invoice-status/${invIndex}`, { status: newStatus })
